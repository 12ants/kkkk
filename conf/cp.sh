...
setOSInfo()
{
  ...
  if [ "$OPERATING_SYSTEM" = 'Linux' ]; then
    if [ -e '/etc/debian_version' ]; then
      ...
      echo "Unable to detect Debian or Ubuntu."
    fi
  else
    echo "Operating System needs to be Linux."
  fi
...
checkIfHostnameResolves()
{
  ...
  if [ -z "${LOCAL_IP}" ]; then
    echo "Hostname $HOSTNAME does not resolve. Set a hosts entry in: /etc/hosts"
  fi
}
...
checkRootPartitionSize()
{
  ...
  if [ $ROOT_PARTITION -lt 7000000 ]; then
    echo "At least 7GB of free hard disk space is required"
  fi
}
...
setupCloudPanel()
{
  ...
  if [ -z "${CLP_RUNNING}" ]; then
    echo "${RED_TEXT_COLOR}CloudPanel couldn't be installed. Check the log above for errors that occurred in the install process.${RESET_TEXT_COLOR}"
  fi
  showSuccessMessage
}
